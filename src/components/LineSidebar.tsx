...
export type LineSidebarProps = {
  ...
  /** Controlled active index (overrides internal selection when provided). */
  activeIndex?: number | null;
  /** When false, items only highlight from cursor proximity (no sticky selection). */
  persistActiveOnClick?: boolean;
...
export default function LineSidebar({
  ...
  const handleClick = useCallback(
    (index: number, label: string) => {
      if (persistActiveOnClick && controlledActive === undefined) {
        setInternalActive(index);
      }
      onItemClick?.(index, label);
    },
    [onItemClick, persistActiveOnClick, controlledActive],
  );
